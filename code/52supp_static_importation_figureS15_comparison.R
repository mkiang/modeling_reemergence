...
supp_result <- dplyr::tbl(con, "read_parquet('supp_analyses/static_importation_summary.parquet')") |>
    dplyr::filter(
        time == 365 * 25 - 1,
...
combined_df <- dplyr::bind_rows(
    ...
    supp_result |>
        dplyr::transmute(pathogen, vaccine_coverage, state, mean = winsorized_mean, p025, p975, result = "static_importation")
) |>
    categorize_vaccine_coverage() |>
    categorize_pathogens() |>
    dplyr::mutate(result_cat = factor(result,
        levels = c("static_importation", "primary"),
        labels = c("Static\nimportation", "Primary result"),
        ordered = TRUE
    )) |>
    dplyr::arrange(pathogen_cat, vaccine_coverage_cat) |>
    dplyr::mutate(vaccine_coverage =
        dplyr::case_when(result == "static_importation" ~ vaccine_coverage - .01,
...
## Save ----
ggplot2::ggsave(
    here::here("plots", "figS15_static_importation_comparison.pdf"),
    p1,
    width = 9,
    height = 8,
    scale = 1,
...
ggplot2::ggsave(
    here::here("plots", "figS15_static_importation_comparison.jpg"),
    p1,
    width = 9,
    height = 8,
    scale = 1,
...
readr::write_csv(
    ...
    here::here("output", "figS15_static_importation.csv")
)
