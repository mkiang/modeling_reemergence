...
combined_df <- dplyr::bind_rows(
    ...
    supp_result |>
        dplyr::transmute(pathogen, vaccine_coverage, state, mean = winsorized_mean, p025, p975, result = "popweighted_import")
) |>
    categorize_vaccine_coverage() |>
    categorize_pathogens() |>
    dplyr::mutate(result_cat = factor(result,
        levels = c("popweighted_import", "primary"),
    ...
    dplyr::mutate(vaccine_coverage =
        dplyr::case_when(result == "popweighted_import" ~ vaccine_coverage - .01,
...
## Save ----
ggplot2::ggsave(
    here::here("plots", "figS16_popweighted_importation_comparison.pdf"),
    p1,
    width = 9,
    height = 8,
    scale = 1,
...
ggplot2::ggsave(
    here::here("plots", "figS16_popweighted_importation_comparison.jpg"),
    p1,
    width = 9,
    height = 8,
    scale = 1,
...
readr::write_csv(
    ...
    here::here("output", "figS16_popweighted_importation.csv")
)
